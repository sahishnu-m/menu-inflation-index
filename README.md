...
**Most selectors are still unverified.** Only 4 of 20 entries in
...
section without a click, an image-based menu with no text to read, or a
menu whose prices are not laid out in a way CSS selectors can isolate. Each
of those entries carries a note explaining what was found. A selector
matching nothing looks exactly like a restaurant that removed its menu, so
the scraper reports the difference and the dashboard shows which are still
unchecked.
